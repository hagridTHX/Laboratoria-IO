...
scene = Scene(
    objects=[
        ...
        Sphere(position=np.array([2.5, 0.5, 2]), radius=1.0, diffuse=np.array([1, 0, 1])),
        Sphere(position=np.array([-2.5, -0.5, 1.0]), radius=0.8, diffuse=np.array([0, 1, 0])),
    ],
    light=Light(position=np.array([3, 2, 5])),
    camera=Camera(position=np.array([0, 0, 10])),
)

rt_recursive = MyRayTracer(scene)
image_recursive = np.clip(rt_recursive.generate_image(), 0, 1)
plt.imshow(image_recursive)
plt.axis("off")
plt.show()